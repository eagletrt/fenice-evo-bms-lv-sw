...
static const uint16_t instances[] = {[CELL_UNDERVOLTAGE] = 6,
                                     [CELL_OVERVOLTAGE] = 6,
                                     [OPEN_WIRE] = 1,
                                     [CAN] = 2,
                                     [SPI] = 1,
...
static const uint16_t timeouts[] = {[CELL_UNDERVOLTAGE] = 400,
...
RingBuffer(Error *, ERROR_INSTANCE_COUNT)
    expired_errors = ring_buffer_new(Error *, ERROR_INSTANCE_COUNT, NULL, NULL);
MinHeap(Error *, ERROR_INSTANCE_COUNT) running_errors =
    min_heap_new(Error *, ERROR_INSTANCE_COUNT, _error_compare);
...
size_t error_get_running(void) { return min_heap_size(&running_errors); }
size_t error_get_expired(void) { return ring_buffer_size(&expired_errors); }
...
void error_reset(ErrorGroup group, ErrorInstance instance) {
  ...
  // Push data to the buffer
  ErrorData data = {
      .group = group, .instance = instance, .timestamp = 0, .op = _error_reset};
  if (ring_buffer_push_back(&err_buf, &data))
    error_routine();
}
...
__attribute__((weak)) void error_update_timer_callback(uint32_t timestamp,
                                                       uint16_t timeout) {}
