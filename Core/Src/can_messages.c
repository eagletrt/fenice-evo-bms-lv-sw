...
void can_init_errors_handler(int can_mgr_error_code) {
    error_simple_set(ERROR_GROUP_BMS_LV_CAN, 0);
}
...
int primary_lv_set_pumps_speed_handler(can_mgr_msg_t *msg) {
    ...
    } else if (pumps_converted.status == primary_lv_set_pumps_speed_status_manual) {
...
int inverters_inv_l_rcv_handler(can_mgr_msg_t *msg) {
    ...
    if (inv_converted.rcv_mux == inverters_inv_l_rcv_rcv_mux_ID_4A_T_Igbt) {
        ...
        float temp_max = fmaxf(inv_temps[0], inv_temps[1]);

        if (radiator_is_auto()) {
            radiator_auto_mode(temp_max);
        }
        if (dac_pump_is_auto()) {
            dac_pump_auto_mode(temp_max);
        }
    }
...
void primary_lv_cells_voltage_send(void) {
    ...
    for (size_t i = 0; i < 2; i++) {
        ...
        ERROR_TOGGLE_IF(can_mgr_send(bms_lv_primary_can_id, &msg) != 0, ERROR_GROUP_BMS_LV_CAN, 0);
    }
}
...
void primary_lv_errors_send(void) {
    ...
    size_t expired_count                    = get_expired_errors();
...
void send_primary_debug_1_signals(float field_1, float field_2, float field_3) {
    primary_debug_signal_1_converted_t converted = {
        .device_id = primary_debug_signal_1_device_id_lv, .field_1 = field_1, .field_2 = field_2, .field_3 = field_3};
