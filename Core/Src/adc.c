...
int current_mux_idx = 0, adc2_conversion_ended = 0, vref_calibration_conversion_ended = 0;
const uint8_t mux_addresses[16]                               = {0, 8, 4, 12, 2, 10, 6, 14, 1, 9, 5, 13, 3, 11, 7, 15};
const uint16_t mux_ctrl_pins[directly_connected_fbs_n_values] = {MUX_A3_Pin, MUX_A2_Pin, MUX_A1_Pin, MUX_A0_Pin};
GPIO_TypeDef *mux_ctrl_ports[directly_connected_fbs_n_values] = {MUX_A3_GPIO_Port, MUX_A2_GPIO_Port, MUX_A1_GPIO_Port, MUX_A0_GPIO_Port};
...
void set_mux_addr(void) {
    ...
    for (size_t midx = 0; midx < 4; midx++) {
        HAL_GPIO_WritePin(mux_ctrl_ports[midx], mux_ctrl_pins[midx], ((cmux_addr >> midx) & 1) ? GPIO_PIN_SET : GPIO_PIN_RESET);
    }
}
    ...
    if (adc2_conversion_ended) {
        if (current_mux_idx < 8) {
            ...
            if (current_mux_idx == mux_sensors_s_hall1_idx) {
                mux_converted_val = CT_get_electric_current_mA(mux_converted_val) - S_HALL1_OFFSET_mA;
            } else if (current_mux_idx == mux_sensors_s_hall2_idx) {
                mux_converted_val = CT_get_electric_current_mA(mux_converted_val) - S_HALL2_OFFSET_mA;
            }
            mux_sensors_mA[current_mux_idx] = mux_converted_val;
        }
        mux_fb_mV[current_mux_idx]   = to_mV(adc2_raw[adc2_ch_mux_fb_idx]) * ADC2_VDM;
        dc_fb_mV[fb_computer_fb_idx] = to_mV(adc2_raw[adc2_ch_not_used_idx]) * ADC2_VDM;
        dc_fb_mV[fb_relay_out_idx]   = to_mV(adc2_raw[adc2_ch_relay_out_idx]) * ADC2_VDM;
        dc_fb_mV[fb_lvms_out_idx]    = to_mV(adc2_raw[adc2_ch_lvms_out_idx]) * ADC2_VDM;
        dc_fb_mV[fb_batt_out_idx]    = to_mV(adc2_raw[adc2_ch_batt_out_idx]) * ADC2_VDM;
